...
public class ApiFailureParserTests
{
    ...
    [InlineData((HttpStatusCode)503, "indisponible")]
    ...
    public void BuildMessage_fallback_selon_statut_si_corps_vide(HttpStatusCode status, string fragmentAttendu)
    {
        ...
        Assert.Contains(fragmentAttendu, msg, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void BuildMessage_Status429_CorpsVide_ContientTropDeRequetes()
    {
        // GIVEN / WHEN / THEN
        var msg = ApiFailureParser.BuildMessage((HttpStatusCode)429, "");
        Assert.Contains("Trop", msg, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void BuildMessage_Status503_CorpsVide_ContientIndisponible()
    {
        var msg = ApiFailureParser.BuildMessage(HttpStatusCode.ServiceUnavailable, "");
        Assert.Contains("indisponible", msg, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void BuildMessage_TimeoutDansCorps_ContientDelai()
    {
        // GIVEN / WHEN / THEN : mot-clé « timeout » déclenche le message utilisateur court délai
        var msg = ApiFailureParser.BuildMessage(HttpStatusCode.BadRequest, "upstream connection timeout");
        Assert.Contains("temps", msg, StringComparison.OrdinalIgnoreCase);
    }
